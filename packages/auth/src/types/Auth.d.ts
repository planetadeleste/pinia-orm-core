...
export interface AuthAxiosRepository extends Request {
  ...
  login(email: string, password: string): Promise<AxiosResponse<Result<ResponseLoginRegisterData>>>;

  restorePassword(email: string): Promise<AxiosResponse<Result<null>>>;

  resetPassword(sCode: string, sPass: string, sPassConfirm: string): Promise<AxiosResponse<Result<null>>>;

  checkResetCode(sCode: string): Promise<AxiosResponse<Result<null>>>;

  logout(sTokenKey ?: string): Promise<AxiosResponse<Result<string>>>;

  register(obData: Record<string, any>): Promise<AxiosResponse<Result<ResponseLoginRegisterData>>>;
