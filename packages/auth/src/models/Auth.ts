...
import type { ResponseCheck, ResponseCsrf, ResponseLoginRegisterData } from '../types';
    ...
    axiosApi: {
      actions: {
        ...
        async login(this: Request, email: string, password: string) {
          const obResponse = await this.post(`${Auth.baseUrl}/login`, { email, password });
          return obResponse.response as AxiosResponse<Result<ResponseLoginRegisterData>>;
        },

        async restorePassword(this: Request, email: string) {
          const obResponse = await this.post(`${Auth.baseUrl}/restore_password`, { email });
          return obResponse.response as AxiosResponse<Result<null>>;
        },

        async resetPassword(this: Request, sCode: string, sPass: string, sPassConfirm: string) {
          const obData = {
            slug: sCode,
            password: sPass,
            password_confirmation: sPassConfirm,
          };
          const obResponse = await this.post(`${Auth.baseUrl}/reset_password`, obData);
          return obResponse.response as AxiosResponse<Result<null>>;
        },

        async checkResetCode(this: Request, sCode: string) {
          const obResponse = await this.get(`${Auth.baseUrl}/reset_password/${sCode}`);
          return obResponse.response as AxiosResponse<Result<null>>;
        },
        ...
        async logout(this: Request, sTokenKey = 'access_token') {
          ...
          const obResponse = await this.post(`${Auth.baseUrl}/invalidate`, obData);
          return obResponse.response as AxiosResponse<Result<string>>;
        },

        async register(this: Request, obData: Record<string, any>) {
          const obResponse = await this.post(`${Auth.baseUrl}/register`, obData);
          return obResponse.response as AxiosResponse<Result<ResponseLoginRegisterData>>;
        },
